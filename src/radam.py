    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                if len(state) == 0:
                    state["step"] = (
                        -4 if self.additional_four else 0
                ...
                if state["step"] > 0:
                    ...
                    state_step = (
                        state["step"] + 4 if self.additional_four else state["step"]
                    )  # since this exp requires exactly 4 step, it is hard coded
                    ...
                    if (
                        ...
                        denom = (
                            exp_avg_sq.sqrt() / math.sqrt(1 - beta2 ** state_step)
                        ).add_(group["eps"])
                        p_data_fp32.addcdiv_(-step_size, exp_avg, denom)
                        p.data.copy_(p_data_fp32)
                    elif self.update_all:
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                if group["warmup"] > state["step"]:
                    scheduled_lr = (
                        1e-6 + state["step"] * (group["lr"] - 1e-6) / group["warmup"]
                    )
                else:
